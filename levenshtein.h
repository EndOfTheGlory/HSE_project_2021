...
void compare_real_learned(const Maze& mz,
    size_t trials,
    const std::vector<std::string> U,
    discrete_vector& DV,
    const std::string& U_1,
    double step,
    size_t eps, std::pair<std::vector<double>, int> final_probs) {
    int last_res = 0;
    auto U_1_int = char_to_int(mz.GetBjn(), std::vector<char>(all(U_1)));

    DV.SetProbs(final_probs.first);
    // G - множество дискретных последовательностей размера (trials + 1)
    std::vector<std::string> G;
    G.reserve(trials + 1);
    size_t N = gen.int_udist(2, 5);
    G = gen_paths_file_str(mz, DV, trials, U_1_int, N);
    std::cout << "Последние реальные пути мыши:\n";
    for (std::string one_real_path : U) {
        std::cout << one_real_path << "\n";
    }
    size_t real_paths_size = U.size();
    std::cout << "\n" << "А теперь то, как мы обучили мышь (последние пути обученной мыши):\n";
    size_t i = 0;
    for (std::string one_learned_string : G) {
        if (i >= G.size() - real_paths_size) {
            std::cout << one_learned_string << "\n";
        }
        ++i;
    }
